...
class Snail:
    ...
    def save_window_screenshot(self, path: str = "screen.png") -> str:
        """Capture the full window and save to disk.

        Returns the file path saved.
        """
        img = self.wait_next_frame()
        cv2.imwrite(path, img)
        logging.info("saved %s (%dx%d)", path, img.shape[1], img.shape[0])
        return path

    def save_non_ui_screenshot(self, path: str = "screen_non_ui.png") -> str:
        """Capture the non-UI area of the window and save to disk.

        Returns the file path saved.
        """
        img = self.wait_next_frame()
        non_ui_img = crop_image(img, self.non_ui_rect)
        cv2.imwrite(path, non_ui_img)
        logging.info("saved %s (%dx%d)", path, non_ui_img.shape[1], non_ui_img.shape[0])
        return path

    def save_center_screenshot(
        self,
        size: int = 100,
        counter: int = 0,
    ) -> str:
        """Capture a centered square crop of the window.

        Parameters
        ----------
        size : int
            Side length of the square crop in pixels (default 100).
        counter : int
            Sequence number for the filename (scrn_{counter:04d}.png).
        ...
        img = self.wait_next_frame()
        r = self.window_rect
        dims = np.array([size, size])
        cent = r.wh() // 2
        rr = Rect.from_centdims(*cent, *dims)
        crop = crop_image(img, rr)
        filename = f"scrn_{counter:04d}.png"
        cv2.imwrite(filename, crop)
        logging.info("saved %s (%dx%d)", filename, crop.shape[1], crop.shape[0])
        return filename
